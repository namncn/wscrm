...
import { getAccountingEmail, sendInvoiceEmailNow } from '@/lib/invoices/mailer'
...
export async function POST(_: NextRequest, { params }: RouteContext) {
  try {
    ...
    const mailResult = await sendInvoiceEmailNow(invoiceId, { cc: ccRecipients })
    ...
    return createSuccessResponse(
      ...
      'Đã gửi hoá đơn thành công tới khách hàng'
    )
  } catch (error) {
    console.error('Error sending invoice email:', error)
    return createErrorResponse('Không thể gửi email hoá đơn', 500)
  }
}
